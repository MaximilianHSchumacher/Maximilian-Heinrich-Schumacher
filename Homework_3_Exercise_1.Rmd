...
# --- Kategorisierung für lnwage ---

cps78$one   <- ifelse(cps78$lnwage <  breaks[2], 1, 0)
cps78$two   <- ifelse(cps78$lnwage >= breaks[2] & cps78$lnwage <  breaks[3], 1, 0)
cps78$three <- ifelse(cps78$lnwage >= breaks[3] & cps78$lnwage <  breaks[4], 1, 0)
cps78$four  <- ifelse(cps78$lnwage >= breaks[4] & cps78$lnwage <  breaks[5], 1, 0)
cps78$five  <- ifelse(cps78$lnwage >= breaks[5] & cps78$lnwage <  breaks[6], 1, 0)
cps78$six   <- ifelse(cps78$lnwage >= breaks[6], 1, 0)

# --- Kategorisierung für wage ---

cps78$onew   <- ifelse(cps78$wage <  breaksw[2], 1, 0)
cps78$twow   <- ifelse(cps78$wage >= breaksw[2] & cps78$wage <  breaksw[3], 1, 0)
cps78$threew <- ifelse(cps78$wage >= breaksw[3] & cps78$wage <  breaksw[4], 1, 0)
cps78$fourw  <- ifelse(cps78$wage >= breaksw[4] & cps78$wage <  breaksw[5], 1, 0)
cps78$fivew  <- ifelse(cps78$wage >= breaksw[5] & cps78$wage <  breaksw[6], 1, 0)
cps78$sixw   <- ifelse(cps78$wage >= breaksw[6], 1, 0)
```
...
```{r}
l <- l <- ggplot(data_long, aes(x = kategorie, y = LN.Wage)) +  # <-- ohne Anführungszeichen!
  geom_col(fill = "red", alpha = 0.6, color = "black", width = 0.7) +
  geom_line(aes(y = X4, group = 1), color = "blue", linewidth = 1.2) +
  geom_point(aes(y = X4), color = "blue", size = 2) +
...
ggsave(filename = "Assignment3/Figures/LNWage.png",plot = l,width = 7,height = 4.5,units = "in",dpi = 300)             
l
```
...
```{r}
pw <- ggplot(data_long, aes(x = kategorie, y = Wage)) +  # <-- ohne Anführungszeichen!
  geom_col(fill = "red", alpha = 0.6, color = "black", width = 0.7) +
  geom_line(aes(y = X4, group = 1), color = "blue", linewidth = 1.2) +
  geom_point(aes(y = X4), color = "blue", size = 2) +
  labs(y = "Observations" ,x=NULL) +
...
```
```{r}
# Chi²-Test für LN(Wage)
chisq_ln <- chisq.test(x = data_long$LN.Wage, p = data_long$X4 / sum(data_long$X4))
chisq_ln

# Chi²-Test für Wage
chisq_w <- chisq.test(x = data_long$Wage, data_long$X4  / sum(data_long$X4))
chisq_w
